...
export function DesktopHomeBranded({ active = "home" }: { active?: AppNavKey }) {
  const catalog = useMultiBrandCatalogBrowser();
    ...
    <main className="min-h-screen bg-[#f7f3eb] text-[#0b1220]">
      ...
      <section className="mx-auto max-w-7xl px-8 pb-14">
        <div className="mb-6 flex items-center justify-between">
          ...
          <span className="rounded-full bg-white px-4 py-2 text-sm font-black text-slate-500 ring-1 ring-[#eee7dc]">Chọn nhiều thương hiệu để so sánh nhanh</span>
        </div>
        ...
        {!catalog.loading && catalog.products.length > 0 ? (
          <>
            ...
            <div className="mt-8 flex flex-col items-center gap-3">
              ...
              {catalog.hasMore ? <button type="button" onClick={catalog.showMore} disabled={catalog.loadingMore} className="rounded-2xl bg-[#ff5a00] px-8 py-4 text-base font-black text-white shadow-lg shadow-orange-200 transition hover:-translate-y-0.5 hover:bg-[#e95000] disabled:cursor-wait disabled:opacity-60">{catalog.loadingMore ? "Đang tải thêm..." : `Xem thêm ${Math.min(catalog.pageSize, catalog.total - catalog.shownCount)} sản phẩm`}</button> : null}
            </div>
          </>
        ) : null}
      </section>
