...
function UploadButton({ label, disabled, busy, onFile }: {
  label: string;
  disabled?: boolean;
  busy?: boolean;
  ...
  return (
    <label className={`inline-flex min-h-11 cursor-pointer items-center justify-center rounded-xl px-4 py-2 text-sm font-black ${disabled ? "cursor-not-allowed bg-slate-200 text-slate-400" : "bg-orange-500 text-white"}`}>
...
export function AdminRecipeOperationsPanelV2() {
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape" && !saving && !uploadingTarget) setEditorOpen(false);
    };
  ...
  }, [editorOpen, saving, uploadingTarget]);
  ...
  async function hydrateCatalogImages(nextForm: FormState): Promise<FormState> {
    const variantIds = [...new Set(nextForm.ingredients.flatMap((item) => item.catalogVariantId ? [item.catalogVariantId] : []))];
    if (variantIds.length === 0) return nextForm;
    try {
  ...
  async function openRecipe(id: string): Promise<void> {
    ...
    try {
      ...
      setForm(await hydrateCatalogImages(toForm(result.recipe)));
  ...
  async function uploadImage(file: File, target: UploadTarget): Promise<void> {
    ...
    const targetKey = target.kind === "cover" ? "cover" : `step:${target.clientId}`;
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "Không upload được ảnh lên R2.");
    } finally {
      setUploadingTarget(null);
    }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {editorOpen ? (
        <div className="fixed inset-0 z-50 bg-slate-950/75 p-0 backdrop-blur-sm md:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving && !uploadingTarget) setEditorOpen(false); }}>
          <section className="mx-auto flex h-full max-w-5xl flex-col overflow-hidden bg-slate-50 text-slate-950 shadow-2xl md:h-[calc(100vh-40px)] md:rounded-[30px]">
            <header className="flex items-start justify-between gap-3 border-b border-slate-200 bg-white p-4 md:p-5">
              ...
              <button disabled={saving || Boolean(uploadingTarget)} onClick={() => setEditorOpen(false)} aria-label="Đóng editor" className="grid h-11 w-11 place-items-center rounded-full bg-slate-100 text-xl font-black disabled:opacity-50">×</button>
            </header>
              ...
              <section className="mt-4 rounded-[24px] bg-white p-4 shadow-sm ring-1 ring-slate-200">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {!lockedForReview ? <UploadButton label="Tải ảnh từ máy" busy={uploadingTarget === "cover"} disabled={Boolean(uploadingTarget && uploadingTarget !== "cover")} onFile={(file) => void uploadImage(file, { kind: "cover" })} /> : null}
              ...
              <section className="mt-4 rounded-[24px] bg-white p-4 shadow-sm ring-1 ring-slate-200">
                ...
                <div className="mt-4 space-y-3">
                  {form.steps.map((item, index) => {
                    const uploadKey = `step:${item.clientId}`;
                    return (
                      <article key={item.clientId} className="rounded-2xl bg-slate-100 p-3 ring-1 ring-slate-200">
            ...
            <footer className="absolute inset-x-0 bottom-0 border-t border-slate-200 bg-white/95 p-3 backdrop-blur md:static md:p-4">
              <div className="mx-auto grid max-w-5xl gap-2 md:grid-cols-3">
                {!lockedForReview ? <button disabled={!canSave || Boolean(uploadingTarget)} onClick={() => void save()} className="rounded-xl bg-orange-500 px-4 py-3 text-sm font-black text-white disabled:opacity-40">{saving ? "Đang lưu..." : form.id ? "Lưu thành bản nháp mới" : "Tạo công thức"}</button> : null}
                {form.id && (form.workflowStatus === "draft" || form.workflowStatus === "changes_requested") ? <button disabled={saving || Boolean(uploadingTarget)} onClick={() => void lifecycle(`/api/admin/recipes/${form.id}/submit-review`)} className="rounded-xl bg-slate-950 px-4 py-3 text-sm font-black text-white disabled:opacity-40">Gửi review</button> : null}
                {form.id && form.workflowStatus === "approved" ? <button disabled={saving || Boolean(uploadingTarget)} onClick={() => void lifecycle(`/api/admin/recipes/${form.id}/publish`)} className="rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white disabled:opacity-40">Xuất bản</button> : null}
              </div>
            </footer>
          </section>
        </div>
