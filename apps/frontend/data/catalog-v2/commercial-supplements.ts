import type { CatalogV2VariantCard } from "./product-model";
...
const BLACK_TEA_DESCRIPTION =
  "Trà đen dùng ủ cốt trà sữa; nước trà nâu đỏ, hương đậm và vẫn giữ mùi khi phối cùng bột sữa.";

function rows(skus: string[], supplement: Supplement) {
  return Object.fromEntries(skus.map((sku) => [sku, supplement]));
}

const supplements: Readonly<Record<string, Supplement>> = Object.freeze({
  ...rows(
    [
  ...
  "BGKQ-0063": {
    ...
    sellUnit: "gói",
    shortDescription: DOUXIAN_DESCRIPTION,
    confidence: "high",
  },
  "BGKQ-0064": {
    sizeLabel: "2 kg",
  ...
  ...rows(
    ["BGKQ-0151", "BGKQ-0152", "BGKQ-0153"],
    {
      ...
      packageLabel: "Gói 500 g",
      sellUnit: "gói",
      shortDescription: BLACK_TEA_DESCRIPTION,
      confidence: "high",
    },
  ),
});

export function enrichCatalogV2Variant(product: CatalogV2VariantCard): CatalogV2VariantCard {
  const supplement = supplements[product.sku];
  if (!supplement) return product;

  const sizeLabel = product.sizeLabel || supplement.sizeLabel || null;
  const packageLabel = product.packageLabel || supplement.packageLabel || null;
  const sellUnit = product.sellUnit || supplement.sellUnit || null;
  const specificationLabel = [
    sizeLabel,
    packageLabel,
    sellUnit ? `ĐVT: ${sellUnit}` : null,
  ...
  return {
    ...
    sellUnit,
    specificationLabel,
    shortDescription: product.shortDescription || supplement.shortDescription,
    commercialDataConfidence: supplement.confidence,
  };
}
