...
import { validateCatalogSelections } from "../src/modules/catalog-v2/catalog-v2-choices";
...
async function main() {
  ...
  }>(`SELECT product_key, catalog_group_key, choice_groups
      FROM catalog_products
      WHERE product_key IN ('siro-mama-gold', 'sinh-to-berrino')`);

  const mama = products.rows.find((row) => row.product_key === "siro-mama-gold");
  const smoothie = products.rows.find((row) => row.product_key === "sinh-to-berrino");
  assert.equal(mama?.catalog_group_key, "siro");
  assert.ok(Array.isArray(mama?.choice_groups) && mama.choice_groups.length === 1);
  ...
  try {
    ...
    const mamaVariant = await db.query<{ id: string }>(
    ...
    const mamaDetailResponse = await fetch(`${baseUrl}/catalog/products/${mamaVariant.rows[0].id}`);
    assert.equal(mamaDetailResponse.status, 200);
    const mamaDetail = await mamaDetailResponse.json() as {
      optionGroups: Array<{ key: string; values: string[] }>;
      choiceGroups: Array<{ key: string; values: string[] }>;
      variants: Array<{ price: number | null; options: Record<string, string> }>;
    };
    assert.ok(mamaDetail.optionGroups.some((group) => group.key === "size" && group.values.length === 2));
    assert.ok(mamaDetail.choiceGroups.some((group) => group.key === "flavor" && group.values.includes("Dâu")));
    assert.equal(new Set(mamaDetail.variants.map((variant) => variant.price)).size, 2);
