...
const remoteDir = `${remoteRoot}/.tmp/catalog-remap-batch-audit-${Date.now()}-${process.pid}`;
...
function fail(message) {
  console.error(`[catalog-remap-batch-audit] ${message}`);
  process.exit(1);
}
...
const auditPath = path.join(here, "audit-catalog-remap-batch.mjs");
if (!fs.existsSync(auditPath)) fail(`Batch audit script not found: ${auditPath}`);
...
function download(remoteName, localPath, label) {
  ...
  console.log(`[catalog-remap-batch-audit] ${label} saved: ${localPath}`);
  return true;
}

console.log(`[catalog-remap-batch-audit] VPS target: ${sshTarget}${remoteRoot}`);
console.log(`[catalog-remap-batch-audit] Task: ${manifest.taskId}; rows=${manifest.rows?.length || 0}`);
console.log("[catalog-remap-batch-audit] Mode: READ ONLY; no migration, SKU update, DB write, R2 write, restart, or deploy.");
...
try {
  ...
  if (!jsonSaved || !csvSaved) throw new Error("Batch audit completed without both report files.");

  const report = JSON.parse(fs.readFileSync(localJsonPath, "utf8"));
  console.log(`[catalog-remap-batch-audit] ${report.status}; rows=${report.summary?.rowCount}; pass=${report.summary?.rowPassCount}; blocked=${report.summary?.rowBlockedCount}`);
  for (const blocker of report.summary?.globalBlockers || []) {
    console.error(`[catalog-remap-batch-audit] Global blocker: ${blocker}`);
  }
  for (const row of report.rows || []) {
    if (!row.pass) {
      console.error(`[catalog-remap-batch-audit] ${row.action} ${row.legacySku || "-"} -> ${row.canonicalSku}: ${(row.blockers || []).join(", ")}`);
    }
  }
  console.log("[catalog-remap-batch-audit] Nothing was written to production.");
  if (report.status !== "BATCH_AUDIT_PASS") process.exitCode = 2;
} catch (error) {
  console.error(`[catalog-remap-batch-audit] VPS audit failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
} finally {
