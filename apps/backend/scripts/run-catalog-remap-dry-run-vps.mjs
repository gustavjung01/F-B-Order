...
function fail(message) {
  console.error(`[catalog-remap-dry-run] ${message}`);
  process.exit(1);
}
...
const manifestPath = resolveRepoPath(argument("manifest", "data/catalog-remap/tea-novia.json"));
...
const dryRunPath = path.join(here, "dry-run-catalog-remap-group.mjs");
if (!fs.existsSync(dryRunPath)) fail(`Dry-run script not found: ${dryRunPath}`);
...
function download(remoteName, localPath, label) {
  ...
  console.log(`[catalog-remap-dry-run] ${label} saved: ${localPath}`);
  return true;
}

console.log(`[catalog-remap-dry-run] VPS target: ${sshTarget}${remoteRoot}`);
console.log(`[catalog-remap-dry-run] Group: ${manifest.groupKey}`);
console.log("[catalog-remap-dry-run] Mode: READ ONLY; no migration, SKU update, DB write, R2 write, restart, or deploy.");
...
try {
  ...
  const remoteScript = [
    ...
    "set +e",
    `node '${dryRunName}' --manifest='${manifestName}' --commercial-file='${commercialName}' --output-json='dry-run.json' --output-csv='dry-run.csv'`,
    "DRY_RUN_STATUS=$?",
    "set -e",
    `printf '%s' "$DRY_RUN_STATUS" > dry-run.status`,
  ...
  console.error(`[catalog-remap-dry-run] VPS dry-run failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
} finally {
