...
async function main() {
  ...
  try {
    ...
    const recipeFixture = fs.readFileSync(
      path.join(repoRoot, "db/fixtures/legacy-recipe-draft.sql"),
      "utf8",
    );
    ...
    const preserved = await db.query(
      `SELECT recipe.slug,recipe.title,recipe.status,recipe.visibility,
    ...
    const ledger = await db.query(
      `SELECT filename FROM schema_migrations
       WHERE filename IN ('db/migrations/009a_recipe_legacy_bridge.sql','db/migrations/010_recipe_core.sql')
       ORDER BY filename`,
    );
    assert.deepEqual(
