...
function sendError(res: Response, error: unknown) {
  ...
  console.error("recipe scale request failed", error);
  res.status(500).json({ error: "RECIPE_SCALE_FAILED" });
}

export function createRecipeScaleRouter() {
  const router = Router();
  ...
  router.post("/:recipeId/scale", async (req, res) => {
    try {
      ...
      const result = await scalePublishedRecipe({
