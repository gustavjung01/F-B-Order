...
for (const moduleName of ["operations", "customers", "orders", "products", "scale", "recipeChrome", "recipePickers", "recipePublish"]) {
  assert.match(source[moduleName], /components\/admin\/ui\//, `${moduleName} must import shared admin UI primitives.`);
}

for (const moduleName of ["customers", "products", "recipePickers"]) {
...
for (const pageName of ["adminPage", "productsPage", "recipesPage", "scalePage"]) {
...
for (const moduleName of ["operations", "customers", "orders", "products", "scale"]) {
...
console.log("Admin design system shell, primitives, module migration, and no-page-CSS contract passed.");
